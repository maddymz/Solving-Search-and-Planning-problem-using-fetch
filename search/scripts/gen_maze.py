...
def generate_blocked_edges(grid_dimension, n_obstacles, seed, myscale=1):
    np.random.seed(seed)
    blocked_edges = set()
    coords = []
    ...
    while(count <= n_obstacles):
        x = myscale*np.random.randint(1, grid_dimension+1)
        ...
        if(flag == 0 and ((x+myscale) <= grid_dimension*myscale) and ((x, y, x+myscale, y) not in blocked_edges)):
            ...
            offset = np.random.uniform(0, myscale)
